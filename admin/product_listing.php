...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $totalRecords = mysqli_query($data, "SELECT * FROM `product` where (".$where.")");
    }else{
    ...
    if(!empty($where)){
        $products = mysqli_query($data, "SELECT * FROM `product` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $products = mysqli_query($data, "SELECT * FROM `product` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    mysqli_close($data);
    ?>
            ...
            <div class="product-search">
                <form id="product-search-form" action="product_listing.php?action=search" method="POST">
                    <fieldset>
                        ...
                        ID: <input type="text" name="id" placeholder="tìm kiếm..." value="<?=!empty($id)?$id:""?>" />
                        Tên sản phẩm: <input type="text" id =search-box name="name" placeholder="tìm kiếm..." value="<?=!empty($name)?$name:""?>" />
