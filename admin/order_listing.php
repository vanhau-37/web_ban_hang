...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $totalRecords = mysqli_query($data, "SELECT * FROM `order` where (".$where.")");
    }else{
        $totalRecords = mysqli_query($data, "SELECT * FROM `order`");
    }
    $totalRecords = $totalRecords->num_rows;
    $totalPages = ceil($totalRecords / $item_per_page);
    if(!empty($where)){
        $order = mysqli_query($data, "SELECT * FROM `order` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $order = mysqli_query($data, "SELECT * FROM `order` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    mysqli_close($data);
    ?>
    <!DOCTYPE html>
